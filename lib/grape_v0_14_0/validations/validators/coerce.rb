  ...
  module Validations
    class CoerceValidator < Base
      def validate_param!(attr_name, params)
        fail GrapeV0_14_0::Exceptions::Validation, params: [@scope.full_name(attr_name)], message_key: :coerce unless params.is_a? Hash
        new_value = coerce_value(params[attr_name])
        if valid_type?(new_value)
          params[attr_name] = new_value
        else
          fail GrapeV0_14_0::Exceptions::Validation, params: [@scope.full_name(attr_name)], message_key: :coerce
        end
      end
